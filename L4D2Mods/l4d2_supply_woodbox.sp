...
ConVar g_hCvarAllow, g_hCvarModes, g_hCvarModesOff, g_hCvarModesTog, g_hCvarMapOff, g_hUseTimers;
...
bool g_bSupplyBoxSpawnFinal, g_bFinaleStarted, g_bUseTimers;
...
public void OnPluginStart()
{
	...
	g_hUseTimers =				CreateConVar(	"l4d2_supply_woodbox_use_timers",	    "1",			"0=Boxes do not spawn on timers, 1=Use timers to spawn boxes", CVAR_FLAGS, true, 0.0, true, 1.0);
	...
	g_hUseTimers.AddChangeHook(ConVarChanged_Cvars);
...
void GetCvars()
{
	...
	g_bUseTimers = g_hUseTimers.BoolValue;
}
...
bool SpawnBoxInRandomLocation(int spawnMax=-1)
{
	...
	if (anyclient > 0)
	{
		...
		if (spawnMax == -1)
			DropNum = GetRandomInt(g_iCvarSupplyBoxMinDrop, g_iCvarSupplyBoxMaxDrop);
		bool bDrop = false;
		float vecPos[3];
