...
{
	CreateNative("Bridge_SpawnZombie", Native_SpawnZombieAuto);
	CreateNative("Bridge_SpawnZombieOfClass", Native_SpawnZombieAutoOfClass);
	CreateNative("Bridge_SpawnMobs", Native_SpawnMobAuto);
	RegPluginLibrary("ZSpawn_Bridge");
...
//									Utils
//==========================================================================================
int GetClassForString(char[] str)
{
	int iClass = -1;
	for (int i = 0; i < g_iZClassAm + 2; i++)	// 3 or 6 zombie classes + tank & witch
	{
		if (g_bL4D2)
		{
			if (StrEqual(g_sZombieClasses_L4D2[i], str, false))
			{
				iClass = i;
				break;
			}
		...
		{
			if (StrEqual(g_sZombieClasses_L4D[i], str, false))
			{
				iClass = i;
				break;
			}	
...
public Action ZSpawnView(int client, int args)
{
	...
	int iClass = GetClassForString(sArgs);
...
public Action ZSpawnAuto(int client, int args)
{
	...
	if (iClass < 0)
	{
		...
		SpawnZombieOfAmount(client, iClass, iAmt);
...
public Action ZSpawnMob (int client, int args)
{
	...
	char sArgs[16];
	GetCmdArg(1, sArgs, sizeof(sArgs));
	int numToSpawn = StringToInt(sArgs);
	SpawnMobOfAmount(client, numToSpawn);
