...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max) 
{
	...
	CreateNative("Bridge_GamePaused", Native_IsAdminPaused);
	CreateNative("Bridge_TogglePaused", Native_ToggleAdminPaused);
	RegPluginLibrary("GamePause_Bridge");
...
any Native_IsAdminPaused(Handle plugin, int numParams)
{
	return g_bIsPaused;
}

any Native_ToggleAdminPaused(Handle plugin, int numParams)
{
	int clientID = 0;
	if (numParams > 0)
		clientID = GetNativeCell(1);
	ToggleAdminPause(clientID);
	return;
}
...
void ToggleAdminPause(int client)
{
	if (client != 0 && GetUserAdmin(client) != INVALID_ADMIN_ID) return;
...
void Pause(int client)
{
	...
	{
		int newClient = GetRandomSurvivor(1, 0);
		FakeClientCommand(newClient, "setpause");
	}
	SetConVarInt(g_hPausable, 0);
	PrintToServer("paused");
...
void Unpause(int client)
{
	...
	{
		int newClient = GetRandomSurvivor(1, 0);
		FakeClientCommand(newClient, "unpause");
	}
	SetConVarInt(g_hPausable, 0);
	PrintToServer("unpaused");
