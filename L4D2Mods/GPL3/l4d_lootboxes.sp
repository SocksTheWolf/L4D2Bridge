...
public APLRes AskPluginLoad2(Handle myself, bool late, char[] error, int err_max)
{
	...
	CreateNative("Bridge_SpawnRandomLootbox", Native_SpawnRandomLootbox_Action);
	CreateNative("Bridge_CleanLootboxes", Native_CleanLootboxes_Action);
	RegPluginLibrary("Lootboxes_Bridge");
...
public void OnPluginStart()
{
	...
	RegAdminCmd("sm_lootbox_spawn", AdminSpawnBox, ADMFLAG_ROOT, "Spawn Lootboxes at your crosshair position.");
 ...
 *                                         Utilities                                         *
 * ========================================================================================= */
bool SpawnBox(int client, int amount=1)
{
	float vPos[3], vAng[3];
	if( !g_bPluginOn ) return false;
	...
	if (client < 1)
	{
		...
		if (anyclient > 0)
		{
			if (L4D_GetRandomPZSpawnPosition(anyclient, 7, 5, vPos) == true)
				vPos[2] += 12;
			else
			{
				ReplyToCommand(client, "%s Couldn't find a valid position for lootbox in 5 tries", CHAT_TAG);
				return false;
			}
		...
		{
			ReplyToCommand(client, "%s Couldn't find a client to spawn to", CHAT_TAG);
			return false;
		}
	}
	else if( !IsValidPosition(client, vPos, vAng) )
	{
		ReplyToCommand(client, "%s Can't spawn a LootBox here, please try again.", CHAT_TAG);
		return false;
	}
...
void ForceCleanAllEnts()
{
	if( !g_bPluginOn ) return;
 ...
 *                                          Natives                                          *
 * ========================================================================================= */
any Native_SpawnRandomLootbox_Action(Handle plugin, int numParams)
{
	int clientID = GetNativeCell(1);
	return SpawnBox(clientID);
}

any Native_CleanLootboxes_Action(Handle plugin, int numParams)
{
	ForceCleanAllEnts();
	return;
}
...
Action AdminSpawnBox(int client, int args)
{
	int iCount;
	char sArgs[8];
	if( args > 0 )
